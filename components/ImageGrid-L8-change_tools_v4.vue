<template>
  ...
  <div class="app-root" :style="rootStyle">
    ...
    <!-- RIGHT: control panel (ancho aumentado a 440px) -->
    <aside class="right-panel">
      <!-- TOP: toolbar con quick save -->
      <div class="toolbar">
        <h3 class="toolbar-title">Controls</h3>
        <button class="quick-save" @click="saveAll" title="Quick save">💾</button>
      </div>

      <!-- SECCION 1: Select folder + file list -->
      ...
      <!-- SECCION 2: Main actions -->
      <section class="panel-section">
        <h4>2. Actions</h4>
        <p class="muted">Primary actions for the current selection.</p>
      ...
      <!-- SECCION 3: View ↔ File mapping -->
      <section class="panel-section">
        <h4>3. View ↔ File</h4>
        <p class="muted">Mapping between view slots and selected files.</p>
      ...
      <!-- SECCION 4: Transformations (reubicada) -->
      <section class="panel-section">
  ...
  <div class="transform-grid">
    <div class="row-btns icon-row">
      <!-- Rotate Left -->
      <button class="btn small icon-btn" @click="applyTransform('rotateLeft')" :disabled="!canTransform" title="Rotate Left">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="icon">
          <path fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                stroke-linejoin="round"
                d="M12 2v4m0 0l-2-2m2 2l2-2M4.93 4.93a10 10 0 1014.14 14.14"/>
        </svg>
      </button>

      <!-- Rotate Right -->
      <button class="btn small icon-btn" @click="applyTransform('rotateRight')" :disabled="!canTransform" title="Rotate Right">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="icon">
          <path fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                stroke-linejoin="round"
                d="M12 2v4m0 0l-2-2m2 2l2-2m6.07 2.93A10 10 0 114.93 19.07"/>
        </svg>
      </button>

      <!-- Flip Horizontal -->
      <button class="btn small icon-btn" @click="applyTransform('flipH')" :disabled="!canTransform" title="Flip Horizontal">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="icon">
          <path fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                stroke-linejoin="round"
                d="M3 12h18M9 5l-5 7 5 7M15 5l5 7-5 7"/>
        </svg>
      </button>

      <!-- Flip Vertical -->
      <button class="btn small icon-btn" @click="applyTransform('flipV')" :disabled="!canTransform" title="Flip Vertical">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="icon">
          <path fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                stroke-linejoin="round"
                d="M12 3v18M5 9l7-5 7 5M5 15l7 5 7-5"/>
        </svg>
      </button>

      <!-- Reset -->
      <button class="btn small icon-btn" @click="applyTransform('reset')" :disabled="!canTransform" title="Reset">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="icon">
          <path fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                stroke-linejoin="round"
                d="M4.93 4.93a10 10 0 1014.14 14.14M12 2v4m0 0l-2-2m2 2l2-2"/>
        </svg>
      </button>
    </div>
    ...
    <div class="row-btns icon-row" style="margin-top:8px;">
      <!-- Zoom In -->
      <button class="btn small icon-btn" @click="applyTransform('zoomIn')" :disabled="!canTransform" title="Zoom In">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="icon">
          <path fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                stroke-linejoin="round"
                d="M12 5v14M5 12h14"/>
      ...
      <!-- Zoom Out -->
      <button class="btn small icon-btn" @click="applyTransform('zoomOut')" :disabled="!canTransform" title="Zoom Out">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="icon">
          <path fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                stroke-linejoin="round"
                d="M5 12h14"/>
    ...
    <div class="nudge-block" style="margin-top:8px;">
      <div class="nudge-row">
        <button class="btn tiny icon-btn" @click="nudgeTransform(0,-10)" :disabled="!canTransform" title="Up">▲</button>
      </div>
      <div class="nudge-row" style="display:flex; gap:6px; justify-content:center; margin-top:6px;">
        <button class="btn tiny icon-btn" @click="nudgeTransform(-10,0)" :disabled="!canTransform" title="Left">◀</button>
        <button class="btn tiny icon-btn" @click="nudgeTransform(10,0)" :disabled="!canTransform" title="Right">▶</button>
      </div>
      <div class="nudge-row" style="display:flex; gap:6px; justify-content:center; margin-top:6px;">
        <button class="btn tiny icon-btn" @click="nudgeTransform(0,10)" :disabled="!canTransform" title="Down">▼</button>
      </div>
      <div class="nudge-hint muted" style="margin-top:6px; font-size:12px;">
        Use nudges to reposition image when zoomed.
      </div>
    </div>
  </div>
</section>

      <!-- STICKY SAVE (al final) -->
...
<script setup>
/*
  ImageGrid.vue — versión estable restaurada + UI tweaks solicitados.
  Comentarios en español; UI en inglés.
  Cambios principales:
   - right-panel ancho = 440px
   - file-list max-height = 200px
   - Load images (antes Reload)
   - Save All al final (sticky) + quick-save arriba
   - conserva TODA la lógica funcional previa (carga, parsing, fallback secuencial, transforms, draw)
*/
