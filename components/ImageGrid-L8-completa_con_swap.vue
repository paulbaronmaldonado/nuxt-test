...
function nudgeTransform(dx, dy) {
  ...
  const angle = -(t.rotate || 0) * Math.PI / 180
  const cosA = Math.cos(angle)
  const sinA = Math.sin(angle)

  // Paso fijo en píxeles de frame
  const step = 0.10
  let frameDx = dx * step * cosA - dy * step * sinA
  let frameDy = dx * step * sinA + dy * step * cosA

  // Neutralizar flip
  if (t.flipH) frameDx *= -1
  if (t.flipV) frameDy *= -1
  ...
  const fw = Math.max(1, Math.round(canvas.clientWidth))
  const fh = Math.max(1, Math.round(canvas.clientHeight))
  const baseScale = Math.min(fw / img.width, fh / img.height) || 1
  const totalScale = baseScale * (t.zoom || 1)

  const deltaX = frameDx / totalScale
  const deltaY = frameDy / totalScale
...
async function saveAll() {
  ...
  for (const s of slots) {
    ...
    // convertir a blob (promisified)
    const blob = await new Promise(resolve => tmpCanvas.toBlob(resolve, 'image/jpeg', 0.95));
    if (!blob) continue;

    // crear URL, link y disparar la descarga de forma segura para Safari
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = fileNames[s] || `${s}.jpg`;

    // Append al DOM (Safari a veces lo requiere)
    document.body.appendChild(a);
    a.click();

    // Espera breve para permitir al navegador procesar la descarga (necesario especialmente en Safari)
    const delayAfterClick = isSafari ? 450 : 80;
    await new Promise(r => setTimeout(r, delayAfterClick));

    // cleanup
    document.body.removeChild(a);

    // revokeObjectURL después de un pequeño retardo extra para evitar revocar antes de que el navegador haya leído la URL
    setTimeout(() => {
      try { URL.revokeObjectURL(url); } catch (e) { /* ignore */ }
    }, 500);
  }

  // opcional: puedes dejar una notificación al final si quieres
